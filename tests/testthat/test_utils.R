test_that("torch_obj_size works with tensors of different dtypes", {
  ...
  expect_equal(torch_obj_size(torch::torch_ones(c(2,7,4))$to(dtype = torch::torch_int())), lobstr:::new_bytes(1790))
  ...
  expect_equal(torch_obj_size(torch::torch_ones(c(2,3,1))$to(dtype = torch::torch_int16())), lobstr:::new_bytes(384))

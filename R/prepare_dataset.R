...
apply_ocr <- function(image) {
  ocr_df <- tesseract::ocr_data(image) %>%
    mutate(poor_word =( str_detect(word, "^\\W+$|\\W{3,}") | confidence <20),
           bb = bbox %>% str_split(",")) %>%
    unnest_wider(bb, names_sep="_") %>%
    filter(!poor_word) %>%
...
#'
#' @param image file path, url, or raw vector to image (png, tiff, jpeg, etc)
#' @param tokenizer tokenizer function to apply to words extracted from image
...
create_features <- function(image,
  ...
  w_h <- image_attributes(original_image)
  ...
  entries <- apply_ocr(original_image) %>%
    mutate(xmin = round(xmin/width * 1000),
           ymin= round(ymin/height * 1000),
           xmax = round(xmax/width * 1000),
           ymax= round(ymax/height * 1000)
    )

  # step 4 tokenize words and get their bbox
  encoding <- tokenizer(entities$word,
                        padding = "max_length",

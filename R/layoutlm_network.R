...
LayoutLMSelfAttention <- torch::nn_module(
    ...
    initialize = function(config, position_embedding_type=NULL){
        stopifnot("The config hidden size is not a multiple of the number of attention " = (config$hidden_size %% config$num_attention_heads == 0 | !is.null(config$embedding_size)))
        ...
        self$position_embedding_type <- append(position_embedding_type,config$position_embedding_type, "absolute")[[1]]
...
LayoutLMModel <- torch::nn_module(
  ...
  forward = function(input_ids=NULL,        bbox=NULL,        attention_mask=NULL,        token_type_ids=NULL,        position_ids=NULL,
        head_mask=NULL,        inputs_embeds=NULL,        encoder_hidden_states=NULL,        encoder_attention_mask=NULL,
        output_attentions=NULL,        output_hidden_states=NULL,        return_dict=NULL){ # -> Unionc(Tuple, BaseModelOutputWithPoolingAndCrossAttentions):
        # r"""
        # Returns:
        #
        # Examples:
